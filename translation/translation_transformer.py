...
for ln in [SRC_LANGUAGE, TGT_LANGUAGE]:
    ...
    vocab_transform[ln] = build_vocab_from_iterator(yield_tokens(data_pipe, ln),
                                                    min_freq=1,
...
def train_epoch(model, optimizer, train_iter, print_step=50):
    model.train()
    losses = 0
    print_loss = 0
    ...
    for src, tgt in train_dataloader:
        ...
        if n % print_step == 0 and n != 0:
            out_loss = print_loss / print_step
            print("[%d][train loss:%5.2f][pp:%5.2f]" % (n, out_loss, math.exp(out_loss)))
            print_loss = 0
...
def evaluate(model, print_step=50):
    model.eval()
    losses = 0
    print_loss = 0
    val_file = "data/val.txt"
    data_pipe = dp.iter.IterableWrapper([val_file])
    data_pipe = dp.iter.FileOpener(data_pipe, mode='rb')
    data_pipe = data_pipe.parse_csv(skip_lines=0, delimiter='\t')
    # 删除行多余数据
    val_iter = data_pipe.map(removeAttribution)
    ...
    for src, tgt in val_dataloader:
        ...
        if n % print_step == 0 and n != 0:
            out_loss = print_loss / print_step
            print("[%d][val loss:%5.2f][pp:%5.2f]" %
                  (n, out_loss, math.exp(out_loss)))
        n = n + 1
...
for epoch in range(1, NUM_EPOCHS + 1):
    start_time = timer()
    train_loss = train_epoch(transformer, optimizer, data_pipe)
    end_time = timer()
    ...
    if not best_val_loss or val_loss < best_val_loss:
        print("[!] saving model...")
        torch.save(transformer.state_dict(), 'transformer_%d_%f.pt' % (epoch, val_loss))
