...
class Inference:
    ...
    def xy(self, data, orig_shape):
        return [
            ops.scale_coords(data.shape[1:], x, orig_shape, normalize=False)
    ...
    def inference(self):
        ...
        if self.st.sidebar.button("Start"):
            ...
            while cap.isOpened():
                ...
                input_box = np.array(xy)
                self.predictor.set_image(frame)
                segs, scores, _ = self.predictor.predict(
                    point_coords=None,
                    point_labels=None,
                    box=input_box[None, :],
                ...
                segments = self.xy(torch.tensor(segs), orig_shape)
                image = Image.fromarray(frame)
                img_draw = ImageDraw.Draw(image)
                np_arr = np.asarray(segments)
                xxyy = np_arr.reshape(-1, 2)
                for xy in xxyy:
                    a = float(xy[0])
                    b = float(xy[1])
                    img_draw.point((a, b), (255, 0, 0))
                # mask_img = self.show_mask(masks[0])
                self.ann_frame.image(image, channels="BGR")  # Display processed frame
